...
use ratatui_image::{picker::{Picker, ProtocolType}, protocol::StatefulProtocol, Image, StatefulImage};
...
pub fn view(frame: &mut Frame, path: String, area: Rect) {
    ...
    let dyn_image = image::ImageReader::open(path).unwrap().decode().expect("dont know");
